...
describe('#breedDetails', () => {
  it('returns the expected description for Bombay', () => {
    const expectedDesc = "The golden eyes and the shiny black coat of the Bombay is absolutely striking. Likely to bond most with one family member, the Bombay will follow you from room to room and will almost always have something to say about what you are doing, loving attention and to be carried around, often on his caregiver's shoulder.";
    const bombay = breedDetails('Bombay');
    assert.equal(expectedDesc, bombay);
  });
  it('returns the expected description for Balinese', () => {
  ...
  it('returns undefined when there is no breed name provided', () => {
    const expectedDesc = undefined
    const none = breedDetails('');
    assert.equal(expectedDesc, none);
  });
